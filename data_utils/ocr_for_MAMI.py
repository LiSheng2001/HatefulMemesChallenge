...
def multi_boxes_mask(image, boxes, pad_crop=5):
    ...
    for box in boxes:
        ...
        pure_white = (patch > 245).all(axis=-1).astype(np.uint8)

        # MAMI里面的迷因既有白色的文字也有黑色的文字，因此添加黑色遮蔽
        pure_black = (patch < 20).all(axis=-1).astype(np.uint8)
        pure_white = pure_white + pure_black
